<div align="center">
  <h1>AUGUSTHON -#30DaysOfPython: Day 28 - API </h1>
...
<small> AUGUSTHON: 28/08/2024 </small>
</sub>

</div>


[<< Day 27](../27_08_2024_Python_with_mongodb/27_python_with_mongodb.md) | [Day 29 >>](../29_08_2024_Building_API/29_building_API.md)
...
conn = http.client.HTTPSConnection("example.com")
conn.request("GET", "/", headers={"User-Agent": "Python HTTP Client"})
response = conn.getresponse()
print(f"Status: {response.status} {response.reason}")
conn.close()
```
...
# GET request
make_request("GET", "/api/resource")

# POST request
make_request("POST", "/api/resource", json.dumps({"key": "value"}))

# PUT request
make_request("PUT", "/api/resource/1", json.dumps({"updated_key": "updated_value"}))

# DELETE request
make_request("DELETE", "/api/resource/1")
```
